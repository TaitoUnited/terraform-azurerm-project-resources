...
resource "azurerm_application_insights_web_test" "uptimez" {
  ...
  name                    = "${var.project}-${var.env}-${values(local.uptimeTargetsById)[count.index].id}"
  location                = data.azurerm_resource_group.namespace.location
  resource_group_name     = data.azurerm_resource_group.namespace.name
  application_insights_id = data.azurerm_application_insights.zone.id
  kind                    = "ping"
  frequency               = 300
  timeout                 = try(values(local.uptimeTargetsById)[count.index].uptimeTimeout, 5)
  enabled                 = true
...
<WebTest Name="WebTest1" Id="ABD48585-0831-40CB-9069-682EA6BB3583" Enabled="True" CssProjectStructure="" CssIteration="" Timeout="0" WorkItemIds="" xmlns="http://microsoft.com/schemas/VisualStudio/TeamTest/2010" Description="" CredentialUserName="" CredentialPassword="" PreAuthenticate="True" Proxy="default" StopOnError="False" RecordedResultFile="" ResultsLocale="">
  <Items>
    <Request Method="GET" Guid="a5f10126-e4cd-570d-961c-cea43999a200" Version="1.1" Url="https://${try(local.ingress.domains[0].name, null)}${values(local.uptimeTargetsById)[count.index].uptimePath}" ThinkTime="0" Timeout="300" ParseDependentRequests="True" FollowRedirects="True" RecordResult="True" Cache="False" ResponseTimeGoal="0" Encoding="utf-8" ExpectedHttpStatusCode="200" ExpectedResponseUrl="" ReportingName="" IgnoreHttpStatusCode="False" />
  </Items>
</WebTest>
XML
}
